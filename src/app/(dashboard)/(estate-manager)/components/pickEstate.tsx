...
const PickEstate = ({ closeRef }: any) => {
    ...
    const { userData, estatesData } = useAuthSlice();
    ...
    return (
        <div ref={closeRef} className='p-4 rounded-[12px] bg-inputBg md:w-[350px] min-w-[340px] mt-[120px] mb-[50px] md:mt-0 md:mb-0'>
            <div className='bg-white rounded-[12px] p-3'>
                <div className='flex gap-2 items-start w-full'>
                    ...
                    <div className='flex flex-col gap-1 w-full'>
                        <span className='text-sm font-medium text-GrayHomz'>{estatesData?.[0]?.basicDetails?.name as any}</span>
            ...
            <div className='flex flex-col gap-2 mt-4'>
                {estatesData && estatesData?.map((data) => (
                <button key={data._id} className='text-GrayHomz text-sm font-normal px-4 py-3 hover:bg-[#E6E6E6] hover:rounded-[4px] text-start'>{data?.basicDetails?.name}</button>
               ))}
            </div>
